...
fetch_repo() {
  ...
  # single-source the protocol wording: extract it from the fetched, generated
  # AGENTS.md rather than maintaining a copy here — scripts/gen-adapters.py is
  # the only place the En protocol summary lives
  PROTOCOL="$(awk 'NR==1{next} /^- \*\*/{exit} {print}' "$SRC/AGENTS.md" | sed 's/^# /## /')"
  case "$PROTOCOL" in
    *"Run En on every incoming task"*) ;;
    *) echo "download looks wrong: En protocol missing from AGENTS.md" >&2; exit 1 ;;
  esac
}
...
Skill bodies live at $NEN_HOME/skills/<name>/SKILL.md — read the engaged
ability's body first: $SKILLS.
<!-- nen:end -->
EOF
}
